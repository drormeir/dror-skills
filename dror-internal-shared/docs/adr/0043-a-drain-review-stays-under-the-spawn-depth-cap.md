...
`dror-implement-ticket` (1) → `dror-review-repair` (2) → `dror-review` (3) —
...
dror-review` three times, each with 1.7–2.8 KB of arguments carrying §0a's
...
**Inlining the review into the loop** — a drain-marked `dror-review-repair`
following `dror-review`'s file in its own context and spawning the lenses and
refuters itself — was implemented first and rejected on review: it keeps the
review's fan-out but loses `dror-repair`'s (the repair agent sits at the cap),
...
a directory override, `dror-implement-ticket` follows `dror-review-repair`'s
...
`dror-review` and `dror-repair` each accept the §0a sentence, run every git
...
**`dror-review` and `dror-repair` gain a directory override and nothing
